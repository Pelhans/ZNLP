...
def test_epoch(dataset, sess, accuracy, cost, X_inputs, y_inputs):
    ...
    for i in xrange(batch_num):
        ...
        feed_dict = {X_inputs:X_batch, y_inputs:y_batch, config_ch.lr:1e-5, config_ch.batch_size:_batch_size, config_ch.keep_prob:1.0}
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        with tf.variable_scope("cws_blstm", reuse=None):
        ...
        for epoch in xrange(max_max_epoch):
            ...
            if epoch > config_ch.max_epoch:
                _lr = _lr * ((decay) ** (epoch - config_ch.max_epoch))
